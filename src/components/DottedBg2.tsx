...
void main() {
  ...
  // Base sampling for fluid distortion
  vec3 baseCol = texture(uTexture, uv).rgb;
  float baseGray = 0.3 * baseCol.r + 0.59 * baseCol.g + 0.11 * baseCol.b;
  
  // Add a slight ripple distortion based on the noise itself to make it feel like liquid
  vec2 distortedUv = uv + vec2(baseGray * 0.03, -baseGray * 0.03);
  
  // Final sampling
  vec3 col = texture(uTexture, distortedUv).rgb;
  float gray = 0.3 * col.r + 0.59 * col.g + 0.11 * col.b;
  
  // Apply gamma to create sharp highlights (water caustics effect)
  gray = pow(clamp(gray, 0.0001, 1.0), uGamma);
  
  // Invert occasionally or just use smoothstep for sharper bands
  // By using smoothstep, we create distinct liquid-like layers
  float mark = smoothstep(0.0, 0.8, gray);
  ...
  if (cnt <= 1) {
    waveCol = uPalette[0];
    waveOpacity = uPaletteAlpha[0];
  } else {
    float scaled = g2 * float(cnt - 1);
    int seg = int(floor(scaled));
    seg = clamp(seg, 0, cnt - 2);
    float f = clamp(scaled - float(seg), 0.0, 1.0);
    waveCol = mix(uPalette[seg], uPalette[seg + 1], f);
    waveOpacity = mix(uPaletteAlpha[seg], uPaletteAlpha[seg + 1], f);
  }
  
  // Continuous water wave opacity instead of dot mark
  fragColor = vec4(waveCol, mark * waveOpacity);
}`;
